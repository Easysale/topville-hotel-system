...
export default function Rooms() {
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center text-lg">Loading rooms...</div>;
  }
  ...
  return (
    <section className="min-h-screen bg-gray-50 py-10 px-6">
      ...
      <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3 max-w-6xl mx-auto">
        {rooms.map((room) => (
          <div key={room.id} className="bg-white rounded-2xl shadow-lg overflow-hidden transition hover:scale-105">
            <img src={room.image_url} alt={room.alt_text} className="w-full h-56 object-cover" />
            <div className="p-5">
              <h3 className="text-xl font-semibold mb-2">{room.title}</h3>
              <p className="text-gray-600 mb-3">{room.description}</p>
              <p className="font-bold text-lg text-blue-700 mb-3">{room.price}</p>
